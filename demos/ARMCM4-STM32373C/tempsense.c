...
ADCConversionGroup adcCalibOffsetGroup = {
  ...
  .ll.sdadc.cr2   = sdadcSTM32ChannelSelect( SDADC_Channel_3 ) | 
                    SDADC_CR2_ADON, 
  .ll.sdadc.jchgr = 0,  /* jchgr */
  .ll.sdadc.conf0r = SDADC_CONFxR_COMMON_VSSA | 
                     SDADC_CONFxR_GAIN_X8 | 
                     SDADC_CONF0R_SE0_0,  /* conf0r */
  .ll.sdadc.conf1r = 0,
  .ll.sdadc.conf2r = 0,
  /* ch3 uses conf0 */
  .ll.sdadc.confchr1 = sdadcSTM32Channel1TO7Config( SDADC_Channel_3, 0),
  .ll.sdadc.confchr2 = 0,
};
...
void takeTemp()
{
  ...
  test_println("Set jumper J18 back to pins 1-2, not 2-3");
  test_println("Then press the key button.");
